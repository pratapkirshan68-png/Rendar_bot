...
async def search_movie(client, msg):
    ...
    # --- 1. SEARCH LOGIC (Spelling Mistake Handle) ---
    
    # Pehle exact search try karega
    cursor = client.movies.find({"title": {"$regex": query, "$options": "i"}})
    results = await cursor.to_list(length=50) # Max 50 files layega taaki bot hang na ho

    # Agar result nahi mila, to "Spelling Fix" try karega (Split Search)
    if not results:
        words = query.split()
        if len(words) > 0:
            # Sabse bada shabd uthayega search ke liye
            keyword = max(words, key=len)
            cursor = client.movies.find({"title": {"$regex": keyword, "$options": "i"}})
            results = await cursor.to_list(length=50)

    if not results:
        await sm.edit(f"❌ `{msg.text}` nahi mili! Spelling thik karke likhein.")
        asyncio.create_task(delete_after_delay([sm], 15))
        return 

    # --- 2. BUTTON MAKE LOGIC (Screenshot Style) ---

    btn_list = []
    
    # Har movie/episode ke liye button banayega
    for res in results:
        db_id = str(res["_id"])
        db_title = res["title"]
        
        # File ka naam jyada lamba ho to chota karega (Button ke liye)
        display_name = db_title[:40] + "..." if len(db_title) > 40 else db_title
        
        # Link generate karna
        bot_url = f"https://t.me/{client.bot_info.username}?start=file_{db_id}"
        final_link = await get_shortlink(bot_url)
        
        # Button add karna list me
        btn_list.append([InlineKeyboardButton(f"🎬 {display_name}", url=final_link)])

    # --- 3. SEND MESSAGE ---

    cap = (f"🎬 **Results for:** `{msg.text}`\n"
           f"👤 **Requested by:** {u_name}")

    # Agar poster chahiye to TMDB use karein, warna simple text message
    poster, _, _, _ = await get_tmdb_info(query)

    if poster:
        res_msg = await client.send_photo(msg.chat.id, poster, caption=cap, reply_markup=InlineKeyboardMarkup(btn_list))
    else:
        res_msg = await client.send_message(msg.chat.id, cap, reply_markup=InlineKeyboardMarkup(btn_list))
    
    await sm.delete()
    # Result ko delete nahi karenge jaldi taki user araam se download kar sake
