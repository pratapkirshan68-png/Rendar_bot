...
# Ye function Movie ke naam me se kachra saaf karega match karne ke liye
def clean_name(text):
    if not text: return ""
    text = text.lower()
    # Remove Quality Tags, Years, Extensions etc.
...
async def get_tmdb_info(query):
    # TMDB search ke liye sirf shuru ke 3-4 shabd use karenge taaki result sahi aaye
...
async def search_movie(client, msg):
    raw_query = msg.text
    query = clean_name(raw_query) # User ki query clean ki
    ...
    # Method 1: Direct Match (Regex)
    try:
        res = await client.movies.find_one({"title": {"$regex": query, "$options": "i"}})
    except: pass

    # Method 2: Advanced Fuzzy Match (Agar direct nahi mila)
    if not res:
        ...
        async for doc in all_movies_cursor:
            ...
            if original:
                # DB Name ko HEAVY clean karo comparison ke liye
                # "Border 2 2026 HDTC" -> "border 2"
                c_title = clean_name(original)
                cleaned_titles.append(c_title)
                # Map cleaned title back to ID
                title_map[c_title] = doc['_id']
        
        # Ab Match karo: "bordar 2" vs "border 2"
        # Cutoff 0.4 means 40% match bhi chalega
    ...
    if res:
        db_id = str(res["_id"]) 
        # TMDB ke liye Hum Clean naam bhejenge, File ka lamba naam nahi
        # Taki poster sahi aaye "Border 2" ka, na ki "Border 2 2026 hdtc..." ka
        search_term_for_tmdb = clean_name(res["title"])
        if len(search_term_for_tmdb) < 2: search_term_for_tmdb = query # Fallback
        ...
        if poster:
            ...
            # disable_web_page_preview=True se bada link box nahi aayega
            res_msg = await client.send_message(msg.chat.id, cap, reply_markup=btn, disable_web_page_preview=True)
            
        await sm.delete() # Searching message delete
...
async def start_handler(client, msg):
    ...
    if data.startswith("file_"):
        ...
        if res:
            ...
            # --- FILE CAPTION (Apki Link Yahan Hai) ---
            caption = (f"🎬 **{title}**\n\n"
...
if __name__ == "__main__":
    ...
    app.run()    return web.Response(text="Bot is Alive & Running")
...
def clean_name(text):
    if not text: return ""
    text = text.lower()
    # Movie ke naam se faltu words hatane ke liye
...
# ================= SEARCH LOGIC (POWERFUL FUZZY) =================

@app.on_message(filters.chat(SEARCH_CHAT) & filters.text & ~filters.command(["start", "pratap", "shortlink", "del"]))
async def search_movie(client, msg):
    raw_query = msg.text
    query = clean_name(raw_query) # User ki spelling clean kar rahe hain
    ...
    if client.movies is None:
        await sm.edit("❌ **Error:** Database Connected Nahi Hai.")
        return

    # 1. Direct Search (Exact Name)
    try:
        res = await client.movies.find_one({"title": {"$regex": query, "$options": "i"}})
    except Exception as e:
    ...
    # 2. Fuzzy Search (Spelling Mistake Handler)
    if not res:
        # DB se saare titles nikalo
        all_movies_cursor = client.movies.find({}, {"title": 1})
        all_titles = []
        title_map = {}
        ...
        async for doc in all_movies_cursor:
            ...
            if original_title:
                clean_t = clean_name(original_title) # DB ka naam bhi clean karo match ke liye
                all_titles.append(clean_t)
                title_map[clean_t] = doc['_id']
        
        # Cutoff 0.4 = 40% match bhi chalega (Bordar -> Border 2 pakad lega)
        matches = difflib.get_close_matches(query, all_titles, n=1, cutoff=0.4)
        
        if matches:
            better_title_clean = matches[0]
            res = await client.movies.find_one({"_id": title_map[better_title_clean]})
            
            await sm.edit(f"💡 **Auto-Correct:** `{raw_query}` ➡️ `{res['title']}`\n✅ **Movie Found!**")
            await asyncio.sleep(1.5)
        
        else:
            # 3. Not Found -> Request Admin
            request_text = (
                f"❌ **Movie Nahi Mili!**\n\n"
                f"👋 Hello {u_mention},\n"
...
async def start_handler(client, msg):
    ...
    # 3. File Sending (Custom Caption Logic)
    data = msg.command[1]
    if data.startswith("file_"):
        if client.movies is None:
        ...
        if res:
            ...
            caption = (f"🎬 **{title}**\n\n"
                       f"❤️ {MAIN_CHANNEL_LINK} ❤️\n"
                       f"Admin : Pratap 🇮🇳\n\n"
                       f"⚠️ **Note:** Ye file 2 minute me delete ho jayegi, jaldi forward kar lo!")
            # ----------------------
